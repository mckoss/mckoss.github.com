...
function WindowControl.prototype.SizeTo(ptSize)
{
	...
	// Early mouse clicks make this line cause an exception???
	try {
...
function Sound.prototype.StUI()
{
	...
	// Hiding the player is subtle.  I tried style display:none, but that was not working in IE 6.0 on Windows ME.
	// Setting WIDTH and HEIGHT to 0 shrink the play to a single pixel on the screen - left as a backup.
	// PARAM tag uiMode=invisible will turn off the display in all versions
	// (I had to remove display:none as it was causing old version to not play audio
	
	st += "<OBJECT WIDTH=0 HEIGHT=0 " + this.StPartID("Player");
	st += " CLASSID='CLSID:6BF52A52-394A-11d3-B153-00C04F79FAA6'>"
	st += " <PARAM NAME=autoStart VALUE=false>";
	st += " <PARAM NAME=uiMode VALUE=invisible>";
	st += "</OBJECT>";
