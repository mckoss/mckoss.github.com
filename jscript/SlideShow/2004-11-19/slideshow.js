...
SlideShow.stRelVersion = "1.0.20041119";
...
function SlideShow(xmlSS, divSS)
{
	...
	sht.addRule("Cache.Main" , stStyle);
...
function SlideShow.prototype.StUI()
{
	...
	for (islide = 0; islide < this.cslides; islide++)
		st += this.rgslide[islide].StCacheUI();
...
MC:
	...
	if (this.islideCur == null)
		{
		if (this.stTransition != "None")
			this.stFilter = stFade;
		break MC;
		}
	...
	switch (this.stTransition)
		{
	case "Wipe":
		this.stFilter = "progid:DXImageTransform.Microsoft.GradientWipe(GradientSize=0.50,duration=0.5,wipestyle=0,motion=";
		
		if (dir == 1)
			this.stFilter += "reverse);";
		else
			this.stFilter += "forward);";
		break;
		}
	}
...
DT: 
	...
	if (this.islideLast != undefined)
		slideLast = this.rgslide[this.islideLast];

	try
		{
		// Last transition still in process - don't start next one until previous transition finished
		if (this.stTransition != "None" && slideLast  && slideLast.imgCache.filters[0].status == 2)
			break DT;
		}
	catch(e) {}
	...
	// Always do layout changes w/o transition effects - resize window mainly
	if (!this.fLayout)
		{
		this.Layout();
		if (slideLast)
			this.PositionSlide(slideLast);
		break DT;
		}

	if (this.islideCur == this.islideLast)
		break DT;

	// Get down to displaying the new slide
	...
	try
		...
		if (slideLast)
			...
			if (this.stFilter)
				{
				slideLast.imgCache.style.filter = this.stFilter;
				slideLast.imgCache.filters[0].apply();
				}
			slideLast.imgCache.style.visibility = "hidden";
			}
		if (this.stFilter)
			{
			slide.imgCache.style.filter = this.stFilter;
			slide.imgCache.filters[0].apply();
			}
		}
	catch (e) {}
	...
	slide.imgCache.style.visibility = "visible";
	...
	try
		...
		if (this.stFilter)
			{
			if (slideLast)
				slideLast.imgCache.filters[0].play();
			slide.imgCache.filters[0].play();
			}
		}
	catch(e) {}
...
function SlideShow.prototype.PositionSlide(slide)
{
	...
	PositionElt(slide.imgCache, this.ptMain);
...
function Slide.prototype.StCacheUI()
{
	var st = "";

	st += "<IMG class=Cache GALLERYIMG=false " + this.StPartID("Cache") +  " SRC=" + StAttrQuote(cbSS.stPath + "images/blank.gif");
...
function Slide.prototype.BindUI()
{
	...
	this.imgCache = this.BoundPart("Cache");
	PositionElt(this.imgCache, new Point(0,0));
	this.SetStatus();
}
...
function Slide.prototype.Preload()
{
	...
	// We're busy loading other images - defer
	if (this.ss.cslidesPre > 0)
		return;
...
function Slide.prototype.PopUp()
{
	...
	var stImgSize = "width: " + ptImage.x + ";" + "height:" + ptImage.y + ";"
	stDoc += "<IMG ID=imgPop SRC=" + StAttrQuote(this.stSrc);
	stDoc += " style=" + StAttrQuote(stImgSize) + ">";

	stDoc += "<IMG onload='DCLargeLoaded();' ID=imgLarge SRC=" + StAttrQuote(this.stLarge);
	stDoc += " style=" + StAttrQuote(stImgSize + "position:absolute;display:none;") + ">";
	...
	stDoc += DCLock;
	stDoc += DCLargeLoaded;
	stDoc += "var tm = setInterval('DCTestLoad();', 500);"
	stDoc += "var pg = new Object;"
	stDoc += "pg.dxImage = " + ptImage.x + ";";
	stDoc += "pg.dyImage = " + ptImage.y + ";";
	stDoc += "</" + "SCRIPT>";
...
function DCTestLoad()
{
	// Wait for util.js to be loaded!
	if (!DragController)
		return;

	clearInterval(tm);

	PositionElt(imgLarge, new Point(0,0));

	dc = new DragController(document.body);

	// Pop-up global variables
	if (pg.imgPop == undefined)
		pg.imgPop = imgPop;

	pg.scale = 1.0;
	pg.ptBaseSize = new Point(pg.dxImage, pg.dyImage);

	pg.wc = new WindowControl(undefined, true);
	DCLock(false);
...
function DCLock(fLock)
{
	...
	if (!fLock)
		{
		pg.wc.Refresh();
		pg.ptSizeLast = pg.wc.ptClientSize.Clone();
		}
}
...
function DCKey()
{
	...
	if (!pg.fLocked)
		{
		if (!pg.ptSizeLast.FEqual(pg.wc.ptClientSize))
			{
			DCLock(true);
			}
		}
...
//		alert(event.keyCode);
	...
	var ptSize = pg.ptBaseSize.Clone();

	ptSize.Mult(pg.scale);

	var ptCenter  = pg.wc.rcClient.PtCenter();
	ptCenter.Mult(dscale);
	var rc = pg.wc.rcClient.Clone();
	...
	if (fScroll)
		pg.imgPop.style.visibility = "hidden";
	SizeElt(pg.imgPop, ptSize);
	if (fScroll)
		{
		pg.wc.ScrollTo(rc.ptUL);
		pg.imgPop.style.visibility = "visible";
		}

	if (!pg.fLocked && pg.wc.FFitsScreen(ptSize) || pg.wc.FFitsWindow(ptSize))
		{
		pg.wc.SizeTo(ptSize);
		DCLock(false);
		}
