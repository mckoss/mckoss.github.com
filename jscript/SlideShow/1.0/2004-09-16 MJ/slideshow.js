...
SlideShow.stRelVersion = "1.0.20040916A";
...
MC:
	...
	switch (this.stTransition)
		{
	case "Wipe":
		var stFilter = "progid:DXImageTransform.Microsoft.GradientWipe(GradientSize=0.50,duration=0.5,wipestyle=0,motion=";
...
Layout:
	...
	if (rcFixed)
		...
		wScale = (rcFixed.Dx() - 2*this.dzBorder) / this.ptSizeL.x;
		if (this.ptSizeL.x != 0 && (this.fUpscale || wScale < 1))
			this.ptSizeL.Mult(wScale);

		wScale = (rcFixed.Dx() - this.rcNav.Dx() - 2*this.dzBorder) / this.ptSizeP.x;
