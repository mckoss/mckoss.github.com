...
function WTDriver.prototype.StUI()
{
	...
	st += "<OBJECT id="+this.stControl+" classid=CLSID:FA13A9FA-CA9B-11D2-9780-00104B242EA3";
	...
	st += "<SCRIPT>" + this.stVarName + ".Init();</SCRIPT>";
...
function WTDriver.prototype.DoKeyboard(wevt)
{
	...
	while (ibag.FNext())
		...
		if (obj.DoKey && !obj.fSuspend)
			obj.DoKey(wevt.getKey(), wevt.getKeyState());
		}		
}
...
function CBController.prototype.DoKey(key, fDown)
{
	if (this.obj.DoKey == undefined)
		return;
	...
	if (ich >= 0)
		{
		this.obj.DoKey(this.stMap.charAt(ich), fDown);
		}
}
...
function Debug.prototype.Trace(st)
{
	...
	if (Debug.prototype.Trace.caller)
		stCaller = Debug.prototype.Trace.caller.StName();
	if (stCaller != "")
		stCaller = "[" + stCaller + "] ";
